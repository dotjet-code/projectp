...
export default function RankingPage() {
  ...
  const maxBuzz = Math.max(...members.map((m) => m.detail.stats.buzz));
  const maxConc = Math.max(...members.map((m) => m.detail.stats.concurrent));
  const maxRev = Math.max(...members.map((m) => m.detail.stats.revenue));

  // Bar chart data
  const barChartMembers = sortedMembers.slice(0, 8);
  const barMax = Math.max(...barChartMembers.flatMap((m) => [m.detail.stats.buzz, m.detail.stats.concurrent, m.detail.stats.revenue]));
              ...
              return (
                <div key={member.id}>
                  ...
                  >
                    ...
                    <div className="w-8 shrink-0 text-center">
                      {member.isTrending && (
                        <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-[#00d492]">
                          ↑
                        </span>
                      )}
                    </div>
