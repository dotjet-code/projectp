...
export default async function AdminStagesPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      <h1 className="text-2xl font-bold mb-1">ステージ管理</h1>
      <p className="text-sm text-gray-600 mb-8">
        特番から次の特番までが1つのステージ。Series は半年の括り。
      </p>

      {/* Active Stage */}
      <section className="mb-10">
        <h2 className="text-lg font-semibold mb-3">現在のステージ</h2>
        {active ? (
          <StageCard stage={active} variant="active" />
        ) : (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-5">
            <p className="text-sm text-gray-600">
              active なステージはありません。下のフォームから作成してください。
            </p>
          </div>
        )}
      </section>
      ...
      {/* Create new */}
      <section className="mb-10">
        <h2 className="text-lg font-semibold mb-3">新しいステージを作成</h2>
        <CreateStageForm hasActive={Boolean(active)} />
      </section>

      {/* Closed Stages history */}
      <section>
        <h2 className="text-lg font-semibold mb-3">過去のステージ</h2>
        {closed.length === 0 ? (
          <p className="text-sm text-gray-500">確定済みのステージはまだありません。</p>
        ) : (
